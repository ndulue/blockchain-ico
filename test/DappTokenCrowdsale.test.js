...
contract('DappTokenCrowdsale', function([_, wallet, investor1, investor2, foundersFund, foundationFund, partnersFund]) {
    ...
    describe('crowdsale', function() {
        ...
        it('tracks the rate', async function () {
            ...
            rate.should.be.bignumber.equal(this.token.rate); 
        });

        
        it('tracks the wallet', async function () {
            const wallet = await this.crowdsale.wallet();
            wallet.should.be.bignumber.equal(this.token.wallet); 
        });
    ...
    describe('crowdsale stages', function() {
        ...
        it('allows admin to update the stage & rate', async function() {
            ...
            const rate = await this.crowdsale.rate();
            stage.should.be.bigNumber.equal(this.icoRate);
        });
    ...
    describe('buyTokens()', function() {
        ...
        describe('when the investor has already met the minimum cap', function () {
            it('allows the investor to contribute below the minimum cap', async function(){
                ...
                // second contribution  is less than investor capp
                const value2 = 1;
        ...
        describe('when the contribution is within the valid range', function () {
            ...
            it('succeeds & updates the contribution amount', async function(){
                await this.crowdsale.buyTokens(investor1, {value: value, from: investor1 }).should.be.fulfilled;
        ...
        describe('token transfers', function () {
            ...
            describe('does not allow investors to transfer tokens during crowdsale', async function () {
                //Buy some tokens first
                await this.crowdsale.buyTokens(investor1, { value: ether(1), from: investor1 });
                // Attempt to transfer tokens during crowdsale
                await this.transfer(investor2, 1, { from: investor1 }).should.be.rejectedWith(EVMRevert);                
            });
        ...
        describe('finalizing the crowdsale', function() {
            ...
            describe('When the goal is reached', function() {
                ...
                it('handles goal reached', async function (){
                    ...
                    //Finishes miniting token
                    const mintingFinished = await this.token.minitingFinished();
                    ...
                    await foundersTimelock.release().should.be.fulfiled;
                    
                    await foundationTimelock.release().should.be.fulfiled;
                    
                    await partnersTimelock.release().should.be.fulfiled;
                    ...
                    await this.vault.refund(investor1, { from: investor }).should.be.rejectedWith(EVMRevert);
